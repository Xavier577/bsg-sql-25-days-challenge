WITH best_goal_scorer AS (
   SELECT 
    ...
    brand_sponsorbrand_used, '"}') AS player  FROM wc_players
...
SELECT 'overall_best_player' AS title, best_player.* FROM best_player
UNION
SELECT 'highest_goal_scorer' AS title, best_goal_scorer.* FROM best_goal_scorer
UNION
SELECT 'highest_assist_provider' AS title, best_assist_provider.* FROM best_assist_provider;